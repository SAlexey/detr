    ...
    def _bbox_ccwh_to_xyxy(bbox:torch.Tensor)->torch.Tensor:
        ...
        if len(ccwh) == 6:
            ...
            xyxy = [
                ...
                ccwh[1] - ccwh[4] * 0.5, # x1
                ccwh[2] - ccwh[5] * 0.5, # y1   
                ccwh[0] + ccwh[3] * 0.5, # z2
                ccwh[1] + ccwh[4] * 0.5, # x2
                ccwh[2] + ccwh[5] * 0.5, # y2   
            ]
        elif len(ccwh) == 4:
            # this is a 2d box
            xyxy = [
                ccwh[0] - ccwh[2] * 0.5, # x1
                ccwh[1] - ccwh[3] * 0.5, # y1   
                ccwh[0] + ccwh[2] * 0.5, # x2
                ccwh[1] + ccwh[3] * 0.5, # y2          
            ]
        else: 
            raise ValueError("Not supported!")
        return torch.stack(xyxy, -1)

    def convert(self, bbox:torch.Tensor, from_fmt:str, to_fmt:str)->torch.Tensor:
        assert {from_fmt, to_fmt}.issubset({"xyxy", "xxyy", "xywh", "ccwh"})
        formatter = getattr(self, f"_bbox_{from_fmt}_to_{to_fmt}", None)
        
        if from_fmt == to_fmt:
            return bbox

        if formatter is None:
            bbox = self.convert(bbox, from_fmt, "xyxy")
            return self.convert(bbox, "xyxy", to_fmt)
